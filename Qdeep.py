...
class Qdeep(Q):
    ...
    #     self.memory_size = 2000  # size of the memory buffer
    #     self.HL_1_size = 24  # number of nodes in the first hidden layer
    #     self.HL_2_size = 24  # number of nodes in the second hidden layer
    #     self.learning_rate_adam = 0.001  # learning rate for Adam optimizer
    #     self.batch_size = 64  # batch size for training
    #     self.min_memory_size = max(self.batch_size, 100)  # minimal memory size before we start training
    #     self.epsilon_initial = 1  # epsilon-greedy policy - initial value
    #     self.epsilon_decay = 999/1000  # decay after each time step
    #     self.epsilon_min =0.01  # minimal value of epsilon
    #
    ...
    def weight_variable(self,name, shape):
        ...
        """
        initial_value = tf.random_uniform(shape=shape,minval=-0.5,maxval=0.5)
    ...
    def take(self, indices):
        ...
        return tf.boolean_mask(self.outputs, mask)

    def build_net(self):
        with tf.name_scope('input'):
            self.x_input = tf.placeholder(dtype=tf.float32, shape=[None, INPUT_SIZE], name='x_input')
            self.action = tf.placeholder(dtype=tf.int32, shape=(None, 3), name="action")
            self.q_est = tf.placeholder(dtype=tf.int32, shape=(None), name="q_estimation")
        ...
        self.loss = tf.reduce_mean(tf.square(self.q_est - self.take(self.action)))
    ...
    def learn(self, prev_state, prev_action, reward, new_state):
        ...
        for i in range(batch_size):
            ...
            prev_states.append(np.array(prev_state).flatten())
            prev_actions.append(prev_action)

            fd_v = {self.x_input: np.array(new_state).flatten().reshape([1, INPUT_SIZE])}
    ...
    def get_action(self, state):
        ...
        """
        fd = {
            self.x_input: state.flatten().reshape([1, INPUT_SIZE])
        }
